...
If the bin width is too wide, we might lose interesting details. If the bin width is too narrow, it might be difficult to get an overall picture of the distribution.
</div>
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(md = median(arr_delay), iqr = IQR(arr_delay))
```
...
nycflights %>% group_by(month) %>% summarise(means = mean(dep_delay)) %>% arrange(desc(means))
```
...
nycflights %>% group_by(month) %>% summarise(medians = median(dep_delay)) %>% arrange(desc(medians))
...
nycflights %>% 
  group_by(origin) %>% 
  summarise(ot_dep_rate = sum(dep_type == "on time") / n()) %>% arrange(desc(ot_dep_rate))
```
...
nycflights%>% group_by(tailnum) %>% summarise(as = avg_speed) %>% arrange(desc(as))
```
...
ggplot(data=nycflights,aes(x=avg_speed,y=distance))+geom_point(alpha=0.3)
```
...
nycflights %>% group_by(dep_type == "delayed") %>% summarise(proportion = sum(arr_type == "on time")/n())
```
