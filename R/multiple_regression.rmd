...
```{r summary-model}
# type your code for the Exercise here, and Knit
aa <- lm(evals$score ~ evals$bty_avg)
...
plot(aa$residuals ~ aa$fitted) + abline(h=0)
plot(abs(aa$residuals) ~ aa$fitted)
hist(aa$residuals)
qqnorm(aa$residuals)
qqline(aa$residuals)
```
...
```{r new-mlr-model}
# type your code for the Exercise here, and Knit
bb <- lm(score ~ bty_avg + rank, data = evals)
summary(bb)
```
