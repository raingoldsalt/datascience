...
samp1 %>% summarise(mean = mean(area), median = median(area), std = sd(area), 
                   IQR = IQR(area), min = min(area), max = max(area), Q1 = quantile(area,0.25),
                   Q2 = quantile(area, 0.75))
```
...
# type your code for the Exercise here, and Run Document
sample_means_small <- ames %>% rep_sample_n(size = 10, replace = TRUE, reps = 25)
sample_means_small %>% summarise(x_bar = mean(area))
```
...
The answer: Practically, the se decreases as the number of sample increase. In addition, the mean moves closer to the true mean. The shape of these would become closer to the sample distribution. If I increased the number of simulations, I would expect the se value to decrease.
</div>
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50 <- ames %>% rep_sample_n(size=50,rep=5000,replace=TRUE) %>% summarise(x_bar=mean(price))
...
# type your code for this Exercise here, and Run Document
sample_means150 <- ames %>% rep_sample_n(size=150,rep=5000,replace=TRUE) %>% summarise(x_bar=mean(price))
ggplot(data=sample_means150 , aes(x=x_bar)) + geom_histogram(binwidth = 100)
```
...
# type your code for this Exercise here, and Run Document
ames %>% sample_n(size=15) %>% summarise(x_bar=mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <- ames %>% rep_sample_n(size=15,rep=2000,replace = TRUE) %>% summarise(x_bar=mean(price))
ggplot(data=sample_means150, aes(x=x_bar))+geom_histogram(binwidth = 100)
```
...
# type your code for this Exercise here, and Run Document
sample_means150 <- ames %>% rep_sample_n(size=150,rep=2000,replace = TRUE) %>% summarise(x_bar=mean(price))
ggplot(data=sample_means150, aes(x=x_bar))+geom_histogram(binwidth = 100)
```
...
sample_means15 %>% summarise(mean = mean(x_bar), se = (sd(x_bar)/sqrt(length((x_bar)))))
sample_means150 %>% summarise(mean = mean(x_bar), se = (sd(x_bar)/sqrt(length((x_bar)))))
```
