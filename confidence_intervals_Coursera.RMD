...
```{r set-seed}
set.seed(20011102)                 # make sure to change the seed
```
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
samp2 <- ames %>% rep_sample_n(size=n, reps = 50, replace = FALSE) %>% summarise(lower=mean(area)-z_star_99*(sd(area)/sqrt(n)),                                                           upper=mean(area)+z_star_99*(sd(area))/sqrt(n))
samp2 <- samp2 %>% mutate(capture_mu = ifelse(lower<params$mu & upper>params$mu, "yes", "no"))
samp2_data <- data.frame(samp2_id = c(1:50, 1:50),
                         samp2_bound = c(samp2$lower,samp2$upper),
                         samp2_capture = c(samp2$capture_mu,samp2$capture_mu))
ggplot(samp2_data, aes(x=samp2_bound,y=samp2_id,group=samp2_id,color=samp2_capture))+geom_point(size =2)+geom_line()+geom_vline(xintercept = params$mu, color = "darkgray")
```
